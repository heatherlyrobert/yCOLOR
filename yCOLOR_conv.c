...
yPALETTE__ryb2hex(           /* PURPOSE = convert RYB angle to RGB hex code   */
      int       a_hue,            /* hue color component              (0-359) */
      char     *a_hex)            /* RGB hex code                   (#RRGGBB) */
{
   ...
   /*---(get hex)--------------------------*/
   strncpy(a_hex, ryb_hex[(int) a_hue], 10);
   return 0;
}
